...
import { type Domain } from './domain-router';
...
const RULES = {
    ...
    browser:
        `Cuando el usuario comparta un enlace o pida leer/resumir una página web o artículo, usa fetch_url con esa URL. Si pide ver cómo se ve una web visualmente, tomar un screenshot, captura de pantalla, o "cómo queda" una URL → usa browser_screenshot (devuelve imagen real del navegador). Si pide automatizar web o formularios (abrir, rellenar campos, click, enviar), usa browser_open/browser_fill/browser_click/browser_submit y finaliza con browser_screenshot_session.`,
    price:
...
const DOMAIN_RULE_KEYS: Record<Domain, readonly RuleKey[]> = {
    desktop:      ['web_search', 'youtube_transcript', 'tts', 'desktop'],
    media:        ['web_search', 'image_gen', 'capture_note', 'remember', 'youtube_transcript', 'tts', 'yt_scripts'],
    comms:        ['web_search', 'capture_note', 'remember', 'tts', 'whatsapp', 'knowledge'],
    linkedin:     ['web_search', 'image_gen', 'capture_note', 'remember', 'linkedin', 'knowledge', 'infographics'],
    tiktok:       ['web_search', 'image_gen', 'youtube_transcript', 'tts', 'tiktok', 'remember'],
...
function getDomainRuleBlock(domain: Domain): string {
    const keys = DOMAIN_RULE_KEYS[domain] ?? DOMAIN_RULE_KEYS.general;
    return keys.map((k) => RULES[k]).join('\n');
}
...
export async function buildSystemPrompt(domain: Domain = 'general'): Promise<string> {
    ...
    const domainRules = getDomainRuleBlock(domain);
...
export async function callLLM(
    ...
    const fullMessages: ChatMessage[] = [
        { role: 'system', content: await buildSystemPrompt(domain) },
        ...messages,
    ];

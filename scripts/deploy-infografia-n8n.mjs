...
const apiKey =
  ...
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMWIyNzU2ZS1mMjk5LTQ4OGUtOWY0Ny05ZWQ4YzMwMzdiZTciLCJpc3MiOiJuOG4iLCJhdWQiOiJwdWJsaWMtYXBpIiwianRpIjoiMjU2YzBiYWItMzEwYS00MTMyLThlOWUtMDRiNDQ0YTIyYWQxIiwiaWF0IjoxNzc5NTcwMjk0fQ.yqNJ2s4J2KK9V1LoTSf0ZqTBudC_0HLXi2VZaBkrf0s';
const base = 'https://ziroxxn8n.ziroxn8n.site/api/v1';
const headers = { 'X-N8N-API-KEY': apiKey, 'Content-Type': 'application/json' };

const { execSync } = await import('child_process');
const wfJson = execSync('node scripts/build-infografia-workflow-payload.js', {
  cwd: root,
  encoding: 'utf8',
});
...
const test = await fetch('https://ziroxxn8n.ziroxn8n.site/webhook/agent-crear-infografia', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
    title: 'Test AgentZirox Infografia',
    subtitle: 'Prueba automatica',
    steps: ['Pide al agente', 'n8n genera guion', 'Guarda en Postgres'],
    benefits: ['Memoria en Obsidian', 'HTML listo', 'job_id real'],
    chat_id: 'test-cursor',
  }),
});
